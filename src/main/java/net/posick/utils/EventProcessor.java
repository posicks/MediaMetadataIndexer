...
/**
 * The EventProcessor class implements a thread safe, listener subsystem that does not 
 * create temporary objects during the event dispatch process. The order in which listeners 
 * are registered determines the order by which the listeners are called during event dispatch.
 * A listener may halt the delivery of events to subsequent listeners by throwing a 
 * StopDispatchException.
 * 
 * @author posicks
 */
@SuppressWarnings("unchecked")
public class EventProcessor<T> implements Closeable
{
    ...
    protected static class Dispatcher implements InvocationHandler
    {
        ...
        protected Dispatcher(final EventProcessor<?> processor)
        {
    ...
    public EventProcessor(final Class<T> iface)
    {
        this.iface = iface;
